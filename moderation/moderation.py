...
class Moderation(commands.Cog):
        ...
        **Notes:**
        ...
        try:
            ...
            await self.logging.send_log(
                ...
                moderator=ctx.author,
                reason=reason,
                description=f"`{member}` has been warned, but was not notified via DM as they have it disabled.",
            )
            raise commands.BadArgument(f"I couldn't send a warn notification to `{member}` on DM's since they have it disabled however the warning has been logged.")
    ...
    @commands.Cog.listener()
    async def on_message_delete(self, *args, **kwargs) -> None:
        await self.logging.on_message_delete(*args, **kwargs)
